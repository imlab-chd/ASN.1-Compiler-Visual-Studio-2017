...
#define WITH_MODULE_NAMESPACE(mod, ns_var, code)     \
    ({    \
        struct asn1_namespace_s *ns_var = asn1_namespace_new_from_module((mod), 1); \
        __typeof__(code) ret = (code);                    \
        asn1_namespace_free(ns_var);                \
        ret;                                        \
    })
//void
//WITH_MODULE_NAMESPACE_FUNC(struct asn1p_module_s *mod, asn1_namespace_t *ns_var,
//                      asn1p_expr_t *code) {                                              
//    ns_var = asn1_namespace_new_from_module(mod, 1); 
//    decltype(code) ret;
//    ret = code;                    
//    asn1_namespace_free(ns_var);                
//    ret;                                        
