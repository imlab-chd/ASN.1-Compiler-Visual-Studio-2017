...
OBJECT_IDENTIFIER_get_first_arcs(const uint8_t *arcbuf, size_t arcbuf_len,
    ...
    size_t rd = OBJECT_IDENTIFIER_get_single_arc(arcbuf, arcbuf_len, &value);
    if(rd <= 0) return rd;
...
size_t
OBJECT_IDENTIFIER_parse_arcs(const char *oid_text, size_t oid_txt_length,
