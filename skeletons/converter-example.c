...
#undef EX_UNAVAILABLE
#define EX_UNAVAILABLE -1
#undef EX_SOFTWARE
#define EX_SOFTWARE 0
#undef EX_USAGE
#define EX_USAGE 1
/*
...
static void *data_decode_from_file(enum asn_transfer_syntax,
                                   ...
                                   size_t suggested_bufsize, int first_pdu);
...
#define RANDOPT "R:"
static size_t random_max_size = 0; /* Size of the random data */
...
static void
CC_PRINTFLIKE(1, 2) DEBUG(const char *fmt, ...) {
    va_list ap;
    if(!opt_debug) return;
    fprintf(stderr, "AD: ");
    va_start(ap, fmt);
...
main(int ac, char *av[]) {
    ...
    size_t suggested_bufsize = 8192; /* close or equal to stdio buffer */
...
buffer_dump() {
    ...
    if(DynamicBuffer.unbits) {
        ...
        fprintf(stderr, " %" ASN_PRI_SSIZE ":%" ASN_PRI_SSIZE "\n",
                (size_t)DynamicBuffer.length - 1,
                (size_t)8 - DynamicBuffer.unbits);
    } else {
...
static void *
data_decode_from_file(enum asn_transfer_syntax isyntax,
                      asn_TYPE_descriptor_t *pduType, FILE *file,
                      const char *name, size_t suggested_bufsize,
                      int on_first_pdu) {
    static uint8_t *fbuf;
    static size_t fbuf_size;
...
static int
write_out(const void *buffer, size_t size, void *key) {
    FILE *fp = (FILE *)key;
    return (fwrite(buffer, 1, size, fp) == size) ? 0 : -1;
}

static int
argument_is_stdin(char *av[], int idx) {
    if(strcmp(av[idx], "-")) {
...
static FILE *
argument_to_file(char *av[], int idx) {
    return argument_is_stdin(av, idx) ? stdin : fopen(av[idx], "rb");
}

static char *
argument_to_name(char *av[], int idx) {
