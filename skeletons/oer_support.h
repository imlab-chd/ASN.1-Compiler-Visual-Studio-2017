...
typedef struct asn_oer_constraints_s {
    ...
    size_t size;    /* -1 (no constraint) or >= 0 */
 ...
 */
size_t oer_fetch_length(const void *bufptr, size_t size, size_t *len_r);

/*
 * Serialize OER length. Returns the number of bytes serialized
 * or -1 if a given callback returned with negative result.
 */
size_t oer_serialize_length(size_t length, asn_app_consume_bytes_f *cb, void *app_key);
