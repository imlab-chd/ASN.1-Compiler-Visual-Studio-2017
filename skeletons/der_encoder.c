...
static size_t der_write_TL(ber_tlv_tag_t tag, ber_tlv_len_t len,
...
der_encode_to_buffer(const asn_TYPE_descriptor_t *type_descriptor,
	...
	if(ec.encoded != -1) {
		assert(ec.encoded == (size_t)(buffer_size - arg.left));
...
#define ASN1_DER_MAX_TAGS_COUNT 4
    ...
    size_t lens[ASN1_DER_MAX_TAGS_COUNT * sizeof(size_t)];
...
der_write_TL(ber_tlv_tag_t tag, ber_tlv_len_t len,
	...
	if(tmp == -1 || tmp > (size_t)sizeof(buf)) return -1;
	size += tmp;
