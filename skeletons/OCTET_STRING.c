 ...
 */
static size_t OCTET_STRING__convert_hexadecimal(void *sptr, const void *chunk_buf, size_t chunk_size, int have_more) {
 ...
 */
static size_t OCTET_STRING__convert_binary(void *sptr, const void *chunk_buf, size_t chunk_size, int have_more) {
	BIT_STRING_t *st = (BIT_STRING_t *)sptr;
...
OCTET_STRING__decode_xer(
    ...
    int (*opt_unexpected_tag_decoder)(void *struct_ptr, const void *chunk_buf,
                                      size_t chunk_size),
    size_t (*body_receiver)(void *struct_ptr, const void *chunk_buf,
                             size_t chunk_size, int have_more)) {
    OCTET_STRING_t *st = (OCTET_STRING_t *)*sptr;
	const asn_OCTET_STRING_specifics_t *specs = td->specifics
				? (const asn_OCTET_STRING_specifics_t *)td->specifics
...
OCTET_STRING_encode_uper(const asn_TYPE_descriptor_t *td,
    ...
    if(csiz->effective_bits >= 0) {
        if((size_t)size_in_units < csiz->lower_bound
           || (size_t)size_in_units > csiz->upper_bound) {
            if(ct_extensible) {
                csiz = &asn_DEF_OCTET_STRING_constraints.size;
                unit_bits = canonical_unit_bits;
                inext = 1;
    ...
    do {
        int need_eom = 0;
        size_t may_save = uper_put_length(po, size_in_units, &need_eom);
...
OCTET_STRING_random_length_constrained(
    ...
    if(constraints->per_constraints) {
        ...
        if(pc->flags & APC_CONSTRAINED) {
            long suggested_upper_bound = pc->upper_bound < (size_t)max_length
                                             ? pc->upper_bound
                                             : (size_t)max_length;
