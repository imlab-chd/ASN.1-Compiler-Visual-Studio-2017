	...
	|| ((memb->expr_type == ASN_BASIC_ENUMERATED
		...
		arg->embed++;
			...
			if(tmp_memb->Identifier == 0) {
				tmp_memb->Identifier = _strdup("Member");
				if(0)
				tmp_memb->Identifier = _strdup(
...
asn1c_lang_C_OpenType(arg_t *arg, asn1c_ioc_table_and_objset_t *opt_ioc,
    ...
    size_t column_index = find_column_index(arg, opt_ioc, column_name);
    if(column_index < 0) {
    ...
    open_type_choice->Identifier = _strdup(arg->expr->Identifier);
...
asn1c_lang_C_type_REFERENCE(arg_t *arg) {
	...
	if(ref->components[ref->comp_count-1].name[0] == '&') {
		...
		if(extract) {
			...
			extract->Identifier = _strdup(arg->expr->Identifier);
...
_add_tag2el_member(arg_t *arg, tag2el_t **tag2el, int *count, int el_no, fte_e flags) {
    ...
    ret = WITH_MODULE_NAMESPACE(arg->expr->module, expr_ns,
...
emit_single_member_PER_constraint(arg_t *arg, asn1cnst_range_t *range, int alphabetsize, const char *type) {
    ...
    if(range->left.type == ARE_VALUE) {
		if(range->right.type == ARE_VALUE) {
			...
			size_t ebits;	/* Value effective range bits */
...
emit_type_DEF(arg_t *arg, asn1p_expr_t *expr, enum tvm_compat tv_mode, int tags_count, int all_tags_count, int elements_count, enum etd_spec spec) {
	...
	char *expr_id = _strdup(MKID(expr));
	char *p = expr_id;
	char *p2 = (char *)0;
	...
	INDENT(+1);
		...
		if(expr->_anonymous_type) {
			p = _strdup(ASN_EXPR_TYPE2STR(expr->expr_type));
			OUT("\"%s\",\n", p?p:"");
			OUT("\"%s\",\n",
		...
		if(expr->expr_type & ASN_CONSTR_MASK) {
			using_type_name = 1;
			p = _strdup(asn1c_type_name(arg, arg->expr, TNF_SAFE));
		} else {
			if (expr->expr_type == A1TC_REFERENCE) {
				p2 = _strdup(asn1c_type_name(arg, terminal, TNF_SAFE));
			} else {
				p2 = _strdup(asn1c_type_name(arg, expr, TNF_SAFE));
			}
			p = _strdup(expr_id);
		}
		if (!p2)
